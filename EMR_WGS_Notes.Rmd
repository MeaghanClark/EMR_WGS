...
- then modify script to output both total ROH and implement a minimum size threshold 
  - need to modify loop such that it is also keeping track of high quality sites within a ROH, output that with the ROH info, then can implement different size cut offs using the length info in the table? Number of total sites and total HQ sites should remain the same. 
- modify running script on hpcc
- re-run for all individuals 
```{bash, eval = TRUE}
